...
class MyAgent(Agent):
    ...
    def next_action(self, last_action):
        if last_action:
            ...
            print("%s moved from %s to %s" % (last_player, str(last_action[0:2]), str(last_action[2:4])))
            # TODO: 1. update your internal world model according to the action that was just executed
            self.env.do_move(self.env.current_state, list(last_action)) # TODO Er þetta málið? Nóg?
        ...
        if self.my_turn:
            ...
            '''
            def negamax_alpha_beta(state, depth, alpha = -124, beta = 124, minmax = 1):
                # self.expanded = 0
                if self.env.is_terminal(state) or depth <= 0:
                    return minmax * self.env.evaluate_state(state), None
                ...
                for move in legal_moves:
                    ...
                    self.expanded += 1
                    self.env.do_move(state, move)
                    value, _ = negamax_alpha_beta(state, depth - 1, -alpha, -beta, -minmax)
            ...
            # try:
            # best_value, best_move = negamax_alpha_beta(copy.deepcopy(_state), 3, -128, 127) # cp because used below
            best_value, bm = minimax_ab(copy.deepcopy(self.env.current_state), 5, -100, 100, self.my_turn) # cp because used below
            #best_value, bm = negamax(copy.deepcopy(self.env.current_state), self.max_depth, self.my_turn)
            time_in_sec = time.process_time() - self.elapsed
            print('\n ALPHABETA\n', '-'*26,
                '\n | BEST VALUE:', best_value,
                '\n | BEST MOVE:', self.best_move, bm,
